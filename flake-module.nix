devenvFlake: { flake-parts-lib, lib, inputs, ... }: {
  options.perSystem = flake-parts-lib.mkPerSystemOption ({ config, pkgs, system, ... }:
...

    {
      ...
      options.devenv.integrations.git-hooks = {
        enable = mkOption {
          type = types.bool;
      ...
      options.devenv.integrations.treefmt = {
        enable = mkOption {
          type = types.bool;
      ...
      config.pre-commit.settings =
          if config.devenv.integrations.git-hooks.enable && hasAttr config.devenv.integrations.git-hooks.shell config.devenv.shells then
            config.devenv.shells.${config.devenv.integrations.git-hooks.shell}.git-hooks
          else
            { };

      config.treefmt =
          if config.devenv.integrations.treefmt.enable && hasAttr config.devenv.integrations.treefmt.shell config.devenv.shells then
            config.devenv.shells.${config.devenv.integrations.treefmt.shell}.treefmt.config
          else
            { };
